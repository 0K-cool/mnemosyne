...
> to v2.1.0** — alpha is superseded.
>
> **Note on v2.1.0:** adds the soft-to-hard escalation surface —
> `mode: warn` hooks, escalation evaluation over audit logs, gated
> rule promotion (`enforce.audit --apply`), webhook notification,
> and the `enforce --sync` retirement pass. New trust
> considerations for that surface are covered in the threat-model
> addendum below; the v2.0.x enforcement guarantees and the v1.1.x
> retrieval/scanner layers are unchanged.
...
### v2.1.0 addendum — escalation surface (June 2026)

v2.1.0 adds one genuinely new trust transition to the T1–T4 chain
above: **T5 — runtime telemetry feeding back into rule source.**

```text
hook (N4)  →  audit log (N5')  →  aggregator  →  memory entry (N2)
        T4'              T5a               T5b
```

Audit logs were previously *output only* (operator-read telemetry).
The escalation evaluator now consumes them as *input* to a decision
that can mutate a rule file (`mode: warn` → `mode: block`) and
regenerate a hook. New surfaces and their standing mitigations:

- **Audit-log poisoning (T5a)** — anything that can append to
  `.claude/logs/*.audit.jsonl` can manufacture `warn` events and
  force a rule across its threshold. Bounded by construction:
  escalation only ever *tightens* enforcement (a forced promotion is
  an availability nuisance, never a bypass primitive). Mitigations:
  `--apply` is operator-gated per rule by default (`--yes` is an
  explicit non-interactive opt-in); READY output names the log path
  for pre-approval review; unparseable timestamps never count toward
  the window.
- **Rule-file mutation (T5b)** — `--apply` is the first code path in
  Mnemosyne that *writes* to a memory entry. Constrained to a
  line-targeted `mode:` flip + consumed `escalation:` block removal;
  atomic tempfile + replace; the rewritten rule must revalidate
  before anything is written; hook regeneration goes through the
  standard `--rule` pipeline (same atomic write, same schema gates).
- **Webhook egress** — `--webhook-url` / `MNEMOSYNE_WEBHOOK_URL` is
  the first network call in the enforcement layer. Scheme
  allow-listed (http/https only — a webhook URL must never become a
  local file/scheme probe), 5s timeout, fail-soft (a dead webhook
  never breaks the audit run), payload contains rule names and
  counts only — no rule bodies, no log contents.
- **`enforce --sync` deletion** — the first code path that *deletes*
  hooks. Eligibility requires the `AUTO-GENERATED by Mnemosyne`
  provenance header (hand-written files in the output dir are
  reported, never touched, even under `--yes`); per-file operator
  confirmation; audit sidecars always preserved; settings.json
  deregistration remains a manual operator act.

Full risk-register entry: `docs/v2-enforcement.md` § Risk register
(row: "Audit-log poisoning forces escalation").
