...
- ~~**Retirement sync (#29)**~~ — shipped: `python -m enforce --sync`
  turns the orphan WARNING into a gated retirement pass. Orphans are
  provenance-checked (only files carrying the `AUTO-GENERATED by
  Mnemosyne` header are ever eligible — hand-written files in the
  output dir are reported, never touched), the missing source rule is
  named, deletion is confirmed per file (`--yes` for non-interactive),
  audit sidecars are always kept, and each deletion prints the
  settings.json deregistration reminder (registration remains an
  operator act — Mnemosyne never edits settings.json). `--sync
  --dry-run` reports with provenance only.
